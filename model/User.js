...
const createLessonModel = async ({ title, description, url, year }) => {
    try {
          if (!title || !description || !url || !year) {
                  throw new Error("All fields are required");
          }

      const addLesson = await Lesson.create({ title, description, url, year })
...
const updateLessonModel = async ({ id, title, description, url, year }) => {
    try {
